...
/**
 * KPI 9 — Recruitment Efficiency Index card header: Weight/title/StatusChip plus the client's
 * own MOF-format FY Target/YTD Target/YTD Actual/Weighted Achievement strip. Split out from the
 * table below so CP007 can show just this part inside a compact, collapsible card (matching
 * CP004's KPI3/KPI4 layout) while RP001 keeps using the full always-expanded scorecard below.
 */
export function RecruitmentIndexCardHeader({
  kpi9,
  weightPct,
  fy,
  periodLabel,
  ...
  periodLabel: string;
  fyTarget: number;
  /** Adds the big YTD Actual / target line CP004's KPI3/KPI4 cards use — off by default so
   * RP001's always-expanded scorecard (which already shows YTD Actual in the strip below)
   * doesn't gain a redundant line it never asked for. */
  headline?: boolean;
}) {
  return (
    <>
      ...
      {headline && (
        <div className="flex items-baseline gap-2 mb-2">
          <span className="tnum font-head text-2xl font-semibold">{kpi9.ytdActual !== null ? `${kpi9.ytdActual.toFixed(1)}%` : "—"}</span>
          <span className="text-sm text-[hsl(var(--pk-ink-faint))]">/ target {kpi9.ytdTarget !== null ? `${kpi9.ytdTarget.toFixed(1)}%` : "N/A"}</span>
        </div>
      )}
      <KpiMetricStrip
        fy={fy}
...
/** The Metric / Weight / Performance Achievement / Score / Computation / Weighted Score table
 * plus operational notes — the detail half of the KPI 9 scorecard, shown always on RP001 and
 * only once its CP007 card is expanded. */
export function RecruitmentIndexTable({ recruitmentIndex }: { recruitmentIndex: RecruitmentMetric[] | undefined }) {
  const totalWeighted = recruitmentIndex?.reduce((s, m) => s + m.weighted, 0) ?? null;

  return recruitmentIndex && totalWeighted !== null ? (
    <>
 ...
 * scorecard: the header strip above, then the detail table. Shared between CP007
 * (Organisational Capacity, this KPI's own perspective screen — via the split pieces above, so
 * its card can collapse) and RP001 (Resource & People, Section B, always expanded) so both stay
 * visually identical and in sync — this was previously two separate, drifting table
 * implementations.
 */
export function RecruitmentIndexScorecard(props: {
  kpi9: KpiResult;
  ...
  return (
    <div className="rounded-lg border border-[hsl(var(--pk-border))] bg-[hsl(var(--pk-surface))] shadow-card p-4">
      <RecruitmentIndexCardHeader kpi9={props.kpi9} weightPct={props.weightPct} fy={props.fy} periodLabel={props.periodLabel} fyTarget={props.fyTarget} />
      <RecruitmentIndexTable recruitmentIndex={props.recruitmentIndex} />
    </div>
  );
}
