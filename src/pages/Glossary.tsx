  ...
  return (
    <div>
      ...
      <div className="rounded-lg border border-[hsl(var(--pk-border))] bg-[hsl(var(--pk-surface))] shadow-card overflow-hidden">
        {filtered.length === 0 ? (
          <p className="text-xs text-[hsl(var(--pk-ink-faint))] px-4 py-6 text-center">No terms match "{query}".</p>
        ) : (
          <div className="divide-y divide-[hsl(var(--pk-border))]">
            {filtered.map((t) => (
              <div key={t.term} className="flex items-start gap-3 px-4 py-2.5">
                <span className="font-mono-pk text-xs font-semibold text-[hsl(var(--pk-accent))] w-16 shrink-0">{t.term}</span>
      ...
      <p className="text-2xs text-[hsl(var(--pk-ink-faint))] mt-3">{filtered.length} of {glossaryTerms.length} terms shown.</p>
    </div>
  );
}
